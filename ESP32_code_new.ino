...
unsigned long lastPublishTime = 0;
unsigned long lastGoodDataTime = 0;
unsigned long lastGoodPublishTime = 0;

const unsigned long publishInterval      = 60000UL;               // 1 min
const unsigned long maxNoDataDuration    = 10UL * 60UL * 1000UL;   // 10 min
const unsigned long maxNoPublishDuration = 10UL * 60UL * 1000UL;   // 10 min
...
// ===== Reboot message control =====
bool rebootMsgPending = true;
bool rebootMsgSent = false;
String rebootReason = "ESP32 rebooted";

// ================= Reboot Message =================
void publishRebootMessage() {
  StaticJsonDocument<128> doc;
  doc["message"] = "sensor reboot";
  doc["reason"] = rebootReason;
  doc["timestamp"] = millis() / 1000;
  ...
  if (client.publish(mqttTopic, buffer)) {
    Serial.print("✅ Reboot message sent: ");
    Serial.println(buffer);
    rebootMsgSent = true;
    rebootMsgPending = false;
  }
}
...
void setupWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    rebootReason = "WiFi connection failed";
    ESP.restart();
  }
}
...
void connectMQTT() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(mqttClientID, mqttUser, mqttPassword)) {
      ...
      // 🚀 Send reboot message ONLY ONCE after MQTT connect
      if (rebootMsgPending && !rebootMsgSent) {
...
// ================= I2C Recovery =================
void recoverI2CBus() {
  Serial.println("⚙️ Recovering I2C bus...");
  pinMode(SCL_PIN, OUTPUT);
  pinMode(SDA_PIN, INPUT_PULLUP);
...
bool initSensor() {
  ...
  Serial.println("✅ SEN55 initialized");
  return true;
}

// ================= Publish Data =================
bool publishData(float pm1, float pm25, float pm4, float pm10,
                 float hum, float temp, float voc, float nox) {
  ...
  if (client.publish(mqttTopic, buffer)) {
    ...
    unsigned long t = millis();   // ✅ SAME reference time
    firstPublishDone = true;
    lastGoodDataTime = t;
    lastGoodPublishTime = t;
    return true;
  }
...
bool readSensorData() {
  ...
  if (error) {
    Serial.print("⚠️ Sensor read failed: ");
    Serial.println(error);
    readFailCount++;
    return false;
  }
  ...
  readFailCount = 0;
  return publishData(pm1, pm25, pm4, pm10, hum, temp, voc, nox);
}
...
void setup() {
  ...
  if (!initSensor()) {
    recoverI2CBus();
    if (!initSensor()) {
      rebootReason = "Sensor init failed on boot";
      ESP.restart();
    }
  }
...
void loop() {
  ...
  if (now - lastPublishTime >= publishInterval) {
    if (!readSensorData() && readFailCount >= maxReadFails) {
      recoverI2CBus();
      if (!initSensor()) {
        rebootReason = "Sensor read failure loop";
        ESP.restart();
      }
      readFailCount = 0;
    }
  ...
  // ---- Watchdogs (FIXED) ----
  if (firstPublishDone) {
    unsigned long watchdogNow = millis();   // 🔥 CRITICAL FIX

    if (watchdogNow - lastGoodDataTime > maxNoDataDuration) {
      rebootReason = "No sensor data for 10 minutes";
      ESP.restart();
    }

    if (watchdogNow - lastGoodPublishTime > maxNoPublishDuration) {
      rebootReason = "No MQTT publish for 10 minutes";
      ESP.restart();
    }
  }
}
